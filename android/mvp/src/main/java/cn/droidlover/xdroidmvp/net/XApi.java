...
public class XApi {
    ...
    public static final long readTimeoutMills = 20 * 1000l;
    ...
    public static <S> S get(String baseUrl, Class<S> service, Interceptor mInterceptor) {
        return getInstance().getRetrofit(baseUrl, true, mInterceptor).create(service);
    }
    ...
    public Retrofit getRetrofit(String baseUrl, boolean useRx, Interceptor mInterceptor) {
        return getRetrofit(baseUrl, null, useRx, mInterceptor);
    }


    public Retrofit getRetrofit(String baseUrl, NetProvider provider, boolean useRx, Interceptor mInterceptor) {
        ...
        Retrofit.Builder builder = new Retrofit.Builder()
                .baseUrl(baseUrl)
                .client(getClient(baseUrl, provider, mInterceptor))
    ...
    public Retrofit getRetrofit(String baseUrl, NetProvider provider, boolean useRx, InputStream[] certificates, InputStream bksFile, Interceptor interceptor) {
        if (Kits.Empty.check(baseUrl)) {
            throw new IllegalStateException("baseUrl can not be null");
        }
        if (retrofitMap.get(baseUrl) != null) return retrofitMap.get(baseUrl);
        ...
        Retrofit.Builder builder = new Retrofit.Builder()
                .baseUrl(baseUrl)
                .client(getClient(baseUrl, provider, certificates, bksFile, interceptor))
                .addConverterFactory(GsonConverterFactory.create());
        if (useRx) {
            builder.addCallAdapterFactory(RxJava2CallAdapterFactory.create());
        }
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider, Interceptor mInterceptor) {
        if (Kits.Empty.check(baseUrl)) {
            throw new IllegalStateException("baseUrl can not be null");
        }
...
//        builder.connectionPool(new ConnectionPool(50, 1, TimeUnit.SECONDS));
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider, InputStream[] certificates, InputStream bksFile, Interceptor mInterceptor) {
        if (Kits.Empty.check(baseUrl)) {
            throw new IllegalStateException("baseUrl can not be null");
        }
        if (clientMap.get(baseUrl) != null) return clientMap.get(baseUrl);
        ...
        OkHttpClient.Builder builder = new OkHttpClient.Builder();
        builder.sslSocketFactory(Https.getSslSocketFactory(certificates, bksFile, "917058965").sSLSocketFactory);
        builder.connectTimeout(provider.configConnectTimeoutMills() != 0
                ? provider.configConnectTimeoutMills()
                : connectTimeoutMills, TimeUnit.MILLISECONDS);
        builder.readTimeout(provider.configReadTimeoutMills() != 0
